  ...
  useEffect(() => {
    if (!plan && fabricRef.current) {
      ...
      const txt = new fabric.Text(
        ...
        { left: 20, top: 20, fontSize: 16, selectable: false, fill: "#bdd9d7ff", fontFamily: "monospace" }
      );
    ...
    <div className="rounded-2xl border border-gray-200 dark:border-gray-600 bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 p-6 shadow-lg dark:shadow-2xl dark:shadow-black/30 transition-all">
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-4">
        <div className="flex items-center gap-2 flex-wrap">
          <button
            onClick={exportPNG}
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-gradient-to-r from-indigo-600 via-indigo-500 to-indigo-700 hover:from-indigo-700 hover:via-indigo-600 hover:to-indigo-800 text-white font-medium text-sm transition-all duration-300 active:scale-95 shadow-md hover:shadow-lg hover:shadow-indigo-500/40 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-900 btn-elegant"
          ...
          <button
            onClick={exportPDF}
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-gradient-to-r from-emerald-600 via-emerald-500 to-emerald-700 hover:from-emerald-700 hover:via-emerald-600 hover:to-emerald-800 text-white font-medium text-sm transition-all duration-300 active:scale-95 shadow-md hover:shadow-lg hover:shadow-emerald-500/40 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 dark:focus:ring-offset-gray-900 btn-elegant"
            title="Download floor plan as PDF document"
            onMouseEnter={(e) => e.target.classList.add('btn-glow-emerald')}
        ...
        <div className="flex items-center gap-2 ml-auto text-sm">
          <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg font-medium ${editable
